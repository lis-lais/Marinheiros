...
export class MongooseSailorRepository implements SailorRepository {
  ...
  async findById(id: string): Promise<Sailor | null> {
    ...
    if (!doc) return null;
    return new Sailor(doc._id.toString(), new Name(doc.firstName, doc.lastName), doc.rank, doc.createdAt);
  }

  async findAll(): Promise<Sailor[]> {
    const docs = await this.model.find().exec();
    return docs.map((d) => new Sailor(d._id.toString(), new Name(d.firstName, d.lastName), d.rank, d.createdAt));
  }
}
